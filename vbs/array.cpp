...
ArrayADT::ArrayADT(ADT * _dimention_list)
{
	this->ht = new HashTable();
	this->dimention = NULL;
	ListADT * list = dynamic_cast<ListADT*>(_dimention_list);
	if(list==NULL || list->l){
		return;
	}
	LongADT * dim = dynamic_cast<LongADT*>(list->l->ToLong());
	if(dim==NULL){
		return;
	}
	this->dimention = list->l;
	long l = (long)dim;
	ADT * next_dimention_list = list->r;
	for(long i=0;i<l;i++)
	{
		this->Set(new LongADT(i),next_dimention_list);
	}
}

ADT* ArrayADT::Retrieve(ADT* _dimention_list)
{
	ADT * result = NULL;
	...
	long l = (long)dim;
	long ll = (long)this->dimention;
	if(l>=ll){
		return result;
	}
	
	result = this->Get(list->l);
	if(list->r!=NULL){
		ADT * next_dimention_list = list->r;
		//result = result->Retrieve(next_dimention_list);
	}
	return result;
}

ADT* ArrayADT::Assign(ADT* _dimention_list,ADT*_adt)
{
	ADT * result = NULL;
	if(this->dimention==NULL){
		return result;
	...
	if(list->r!=NULL){
		ADT * next_dimention_list = list->r;
		//这里应该根据
		//result = result->Assign(next_dimention_list);
	}else{
		this->Set(list->l,_adt);
	}
	return _adt;
}
