...
vector<string> real_explode(const string &delimiter, const string &str)
{
    int len = str.length();
...
ADT* explode( ADT * _delimiter ,ADT *_str,ADT * _limit )
{
	...
	StrADT * delimiter =dynamic_cast<StrADT*>(_delimiter);
	if(delimiter==NULL){
		return result;
	}
	string delimiter_s = delimiter->Str();
	StrADT * str =dynamic_cast<StrADT*>(_str);
	if(str==NULL){
		return result;
	}	
	string str_s = str->Str();
	vector<string> sv = real_explode(delimiter_s,str_s);
	for (size_t i=0; i<sv.size(); i++)
	{
		result->Append(new StrADT(sv[i]));
	}
	return result;
}
